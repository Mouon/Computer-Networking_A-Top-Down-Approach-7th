...
패킷이 라우터 A 에 도착하게 되면 라우터 A 는 패킷의 헤더를 조사하여 어디로 패킷을 전송할지 결정하고 거기로 전송해주는 역할을 하는데,
...
나는 200Mbps 만 처리할 수 있는데 1000Mbps 의 패킷을 전송해야한다면 200Mbps 씩 5번 전송해야하므로 여기서 발생하는 **전송지연** 은
