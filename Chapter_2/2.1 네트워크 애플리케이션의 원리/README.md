...
애플리케이션 계층 프로토콜은 네트워크 애플리케이션의 일부일 뿐이다. 예로, 웹이라고 하는 건 웹 브라우저, 웹 서버, HTML 문서, 그리고 HTTP 프로토콜 등 여러 요소로 구성되어 있다.
일부 애플리케이션 계층 프로토콜은 공개되어 있고, 일부는 비공개 이다. 공개되어 있는 공개 표준을 RFC 라고 한다.
